...
def matriz_agroecologica(paisaje, matriz_interacciones, tasas_reproduccion, condiciones_iniciales, t_total, Dispersion, Mortalidad, pasos_mm=2):
    """Modelando la matriz agroecológica. Los pasos que sigue el modelo son: dinámica de poblaciones Lotka-Volterra -> (migración -> muerte) x pasos_mm. La población se guarda en cada iteración de Lotka Volterra y en cada iteración de migración+muerte. Regresa un arreglo de numpy con la poblacion en cada paso, con forma:
    ...
    # Inicializar un arreglo de numpy de la forma: poblacion = [tiempo] [x][y] [especies]
...
def migracion(X, tp, L):
    ...
    diccionario L
    L['b'] = valor bosque
    L['m'] = valor milpa / agricultura alta calidad
    L['i'] = valor intensivo / agricultura baja calidad
    X es la matriz de dimensión 3 X(x,y,i) donde 
    la especie i-esima tiene su representación para 
    todos los parches
    tp es la distribución de tipos de parche en todo el 
    espacio
    """
    s = X.shape
    t = np.array(tp)
...
def genera_poblacion_inicial(paisaje, n_especies, p0_bosque, p0_milpa=0, p0_intensivo=0):
    ...
    Recibe: 
        ...
        n_especies: int, la cantidad de especies
